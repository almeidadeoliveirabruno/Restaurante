...
{
    public partial class VisualizarCozinheiros : FormBase
    {
        public VisualizarCozinheiros()
        {
            InitializeComponent();
        ...
        private void btnRemover_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                int idCozinheiro = (int)selectedRow.Cells["IdCozinheiro"].Value;
                // Verifica se o cozinheiro existe no repositório
                if (RepositorioCozinheiros.Cozinheiros.Any(c => c.IdCozinheiro == idCozinheiro))
                {
                    RepositorioCozinheiros.Remover(idCozinheiro); 
                    dataGridView1.DataSource = RepositorioCozinheiros.Cozinheiros; 
                    MessageBox.Show("Cozinheiro removido com sucesso.", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Cozinheiro não encontrado.", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Por favor, selecione um cozinheiro para remover.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
